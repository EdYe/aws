...
###################################
# Following is only for controller
###################################

# Setup X Window System
sudo yum install  xorg-x11-server-Xorg xorg-x11-xauth xorg-x11-apps libXtst -y
# Update /etc/ssh/sshd_config for X Window System
sudo aws s3 cp s3://cobalt-chc-dev-dataplatform/users/UPI_TAIWAN_OFFSHORE/share/dev_software/config/x-windows/sshd_config /etc/ssh/sshd_config
